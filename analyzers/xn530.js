...
let inDir = path.resolve(process.cwd(), 'log', 'XN530_in.txt');
let resDir = path.resolve(process.cwd(), 'log', 'XN530_out.txt'); 
...
const requestMessage = async ( lab_id, socket, orderType = 'Q' ) => {
    ...
    try {
        ...
        let record = `${STX}1H|\\^&|||XN530^1|||||host|TSDWN^REPLY|P|1${CR}`;
        record    += `P|1${CR}`;
...
const server = net.createServer( (socket) => {
    ...
    console.log('XN530 client connected to port ' + PORT);
...
const saveResult0 = async result  => {
    ...
    try {
        ...
        io.emit("xn530_result", { data: result });
...
exports.start = async (io, conn, sghConn) => {
    ...
    server.listen(PORT, () => {
        console.log(`\nxn530 host tcp server listening on ${PORT} `.bgGreen);
    }); 
};

// server.listen(PORT, () => {
//     console.log(`XN530 host tcp server listening on ${PORT} `.green);
// });